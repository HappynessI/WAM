...
class HDF5VLADataset:
    ...
    def __init__(self, model_config_path, wm_horizon) -> None:
        ...
        if "data_paths" in model_config:
            ...
            if isinstance(data_paths, str):
                ...
                data_paths = [path.strip() for path in data_paths.split(",")]
        else:
            data_paths = [model_config["data_path"]]
    ...
    def get_item(self, index: int = None, state_only=False):
        ...
        while True:
            ...
            valid, sample = (self.parse_hdf5_file(file_path, self.wm_horizon)
    ...
    def parse_hdf5_file(self, file_path, wm_horizon):
        ...
        with h5py.File(file_path, "r") as f:
            ...
            lower_bound = max(0, first_idx - 1)

            if lower_bound >= num_steps - 1:
                if num_steps > 0:
                    step_id = num_steps - 1
                else:
                    return False, None
            else:
                step_id = np.random.randint(lower_bound, num_steps)
    ...
    def parse_hdf5_file_state_only(self, file_path):
        ...
        with h5py.File(file_path, "r") as f:
            ...
            if len(indices) > 0:
                ...
                # print(f"Warning: No significant movement detected in {file_path}. Defaulting to use from the first frame.")
                first_idx = 0 
                # raise ValueError("Found no qpos that exceeds the threshold.")
