...
def create_model(args, **kwargs):
    ...
    ema_model_path = kwargs.pop("ema_model_path", None)
    
    model = RoboticDiffusionTransformerModel(args, **kwargs)
    
    if ema_model_path is not None and os.path.exists(ema_model_path):
        print(f"Loading EMA weights from: {ema_model_path}")
        model.ema_model_path = ema_model_path  
        model.load_pretrained_weights(ema_model_path)
    elif kwargs.get("pretrained", None) is not None and os.path.isfile(kwargs.get("pretrained", None)):
        model.load_pretrained_weights(kwargs.get("pretrained", None))
